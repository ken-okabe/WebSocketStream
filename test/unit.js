...
define(['assert2', 'events', 'stream', 'util', '../lib/main'],
    function(assert, events, stream, util, WebSocketStream) {
    ...
    clientSocket.addEventListener('open', function () {
        ...
        // Pretend that the buffer is flushed in 200 ms.  We should see a
        // 'drain' event.
        setTimeout(function () {
            clientSocket.bufferedAmount = 0;
        }, 200);
    ...
    // This should emit a drain event because 'write' returned false when we
    // wrote before it was open.
